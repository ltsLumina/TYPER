...
public class InventoryItem : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler
{
	...
	public void OnDrag(PointerEventData eventData) { transform.DOMove(eventData.position, 0.1f).SetId("drag").SetLink(gameObject); }

	public void OnBeginDrag(PointerEventData eventData)
	{
		canvasGroup.blocksRaycasts = false;

		var lastKeysInCombo = KeyManager.Instance.FlatKeys.Where(k => k.ComboEffect != null);
	...
	public void OnEndDrag(PointerEventData eventData)
	{
		...
		if (keyUI != null) { Destroy(gameObject); }
	}
}
