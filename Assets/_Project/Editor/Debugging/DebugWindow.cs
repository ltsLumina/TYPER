	...
	static void DrawConsoleCommandMenu()
	{
		...
		using (new VerticalScope("box"))
		{
			...
			if (commandsFoldout)
			{
				...
				effectName = TextField(effectName, 32, Height(25));

				bool disabled = !string.IsNullOrEmpty(effectName) && !effectName.Equals(defaultText, StringComparison.OrdinalIgnoreCase);
