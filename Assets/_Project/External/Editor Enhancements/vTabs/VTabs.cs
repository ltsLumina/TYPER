...
public static class VTabs
{
    static void Update()
    {
        ...
        void dragndrop_()
        {
            ...
            if (EditorWindow.mouseOverWindow.GetType() == hierarchyType && DragAndDrop.objectReferences.Any(r => r is GameObject)) dropAreaHeight = 20;
        ...
        void scrolling()
        {
            ...
            if (curEvent.type != EventType.ScrollWheel && VTabsMenuItems.fixPhantomScrollingEnabled && curEvent.delta.x == (int) curEvent.delta.x)
            ...
            void shiftscroll()
            {
                ...
                var dScroll = Application.platform == RuntimePlatform.OSXEditor
                    ? curEvent.delta.x                     // osx sends delta.y as delta.x when shift is pressed
                    : curEvent.delta.x - curEvent.delta.y; // some software on windows may do that too

                if (dScroll != 0)
                    if (curEvent.control || curEvent.command)
                    {
                        if (VTabsMenuItems.shiftscrollMoveTabEnabled) moveTab(dScroll > 0);
                    }
                    else
                    {
                        if (VTabsMenuItems.shiftscrollSwitchTabEnabled) switchTab(dScroll > 0);
                    }
            }
            ...
            void sidescroll()
            {
                ...
                if ((int) (sidesscrollPosition * VTabsMenuItems.sidescrollSensitivity / 2) != (int) ((sidesscrollPosition += curEvent.delta.x) * VTabsMenuItems.sidescrollSensitivity / 2))
                    if (curEvent.control || curEvent.command)
                    {
                        if (VTabsMenuItems.sidescrollMoveTabEnabled) moveTab(curEvent.delta.x < 0);
                    }
                    else
                    {
                        if (VTabsMenuItems.sidescrollSwitchTabEnabled) switchTab(curEvent.delta.x < 0);
                    }
            }
    ...
    static void OnKeyEvent()
    {
        void keyPressed_()
        {
            ...
            if (Event.current.type == EventType.KeyDown) keyPressed = true;

            if (Event.current.type == EventType.KeyUp) keyPressed = false;
        }
        ...
        void addTab()
        {
            ...
            var curTabAlreadyAdded = defaultList.Any(r => r.menuItemName == curTab.titleContent.text.Replace("/", " \u2215 ").Trim(' ')) || customList.Any
                (r => r.menuItemName == curTab.titleContent.text.Replace("/", " \u2215 ").Trim(' '));
            ...
            foreach (var tab in defaultList) menu.AddItem(new GUIContent(tab.menuItemName), false, () => tab.CreateWindow(GetDockArea(curTab), false));

            menu.AddSeparator("");

            foreach (var tab in customList) menu.AddItem(new GUIContent(tab.menuItemName), false, () => tab.CreateWindow(GetDockArea(curTab), false));

            menu.AddSeparator("");

            if (!curTabAlreadyAdded) menu.AddItem(new GUIContent("Add current tab to list"), false, addCurrentTabToList);

            foreach (var tab in customList) menu.AddItem(new GUIContent("Remove from list/" + tab.menuItemName), false, () => removeFromList(tab));
    ...
    static void UpdateTabHeader(EditorWindow window)
    {
        ...
        if (icon) titleContent = new GUIContent(name, icon);
        else titleContent      = new GUIContent("  " + name + "");

        window.titleContent = titleContent;
    }
    static void UpdateBrowserTabHeaders()
    {
        foreach (var browser in lockedBrowsers) UpdateTabHeader(browser);
    }
    static void UpdateInspectorTabHeaders()
    {
        foreach (var insepctor in lockedInspectors) UpdateTabHeader(insepctor);
    }
...
#if UNITY_2020_1_OR_NEWER
            var width = (float) getWidth.Invoke
            (dockArea, new[]
             { tabStyle, tab });
#else
    ...
    static void OnSceneOpened(Scene scene, OpenSceneMode mode) => UpdateInspectorTabHeaders();
    ...
    class TabDescription
    {
        ...
        public TabDescription(EditorWindow window)
        {
            ...
            if (window.GetType() == inspectorType)
                if (inspectorType.GetMethod("GetInspectedObject", maxBindingFlags).Invoke(window, null) is Object obj)
                {
                    ...
                    if (path != "") objectGuid = AssetDatabase.AssetPathToGUID(path);
                    else objectInstanceId      = obj.GetInstanceID();
        ...
        public EditorWindow CreateWindow(object dockArea, bool atStoredTabIndex = true)
        {
            ...
            if (atStoredTabIndex)
            {
                ...
                dockAreaType.GetMethods(maxBindingFlags).First(r => r.Name == "AddTab" && r.GetParameters().Count() == 3).Invoke
                (dockArea, new[]
                 { tabIndex, (object) window, true });
            }
            else
                dockAreaType.GetMethods(maxBindingFlags).First(r => r.Name == "AddTab" && r.GetParameters().Count() == 2).Invoke
                (dockArea, new[]
                 { (object) window, true });
            ...
            if (isBrowser)
            {
                ...
                if (s_LastInteractedProjectBrowser != null)
                {
                    ...
                    if (listAreaSource != null && listAreaDest != null) pi_gridSize.SetValue(listAreaDest, pi_gridSize.GetValue(listAreaSource));
                }
                ...
                if (objectGuid != "")
                {
                    ...
                    if (isLockedToObject) browserType.GetProperty("isLocked", maxBindingFlags).SetValue(window, true, null);
                }
            }
            ...
            if (isInspector)
            {
                ...
                if (objectInstanceId != 0) obj = EditorUtility.InstanceIDToObject(objectInstanceId);
                else obj                       = AssetDatabase.LoadAssetAtPath<Object>(AssetDatabase.GUIDToAssetPath(objectGuid));
            ...
            if (isBrowser) UpdateTabHeader(window);

            window.Focus();

            if (isInspector) UpdateInspectorTabHeadersPreventingReset();
    ...
    static Type dockAreaType => typeof(Editor).Assembly.GetType("UnityEditor.DockArea");
    static Type inspectorType => typeof(Editor).Assembly.GetType("UnityEditor.InspectorWindow");
    static Type browserType => typeof(Editor).Assembly.GetType("UnityEditor.ProjectBrowser");
    static Type hierarchyType => typeof(Editor).Assembly.GetType("UnityEditor.SceneHierarchyWindow");
