...
public static class VFolders
{
    static void ItemGUI(string guid, Rect rect)
    {
        ...
        {
            if (eType == EventType.Repaint && isSingleLine) listviewAssetRectX = EditorGUIUtility.GUIToScreenPoint(rect.position).x;
        ...
        void folderData_()
        {
            if (!data.folderDatasByGuid.ContainsKey(guid)) data.folderDatasByGuid.Add(guid, new FolderData(guid));
