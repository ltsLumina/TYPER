...
public class Fractal : MonoBehaviour
{
    ...
    void ApplyRenderingSettings()
    {
        foreach (var meshRenderer in GetComponentsInChildren<MeshRenderer>())
        {
            ...
            meshRenderer.shadowCastingMode               = shadows ? ShadowCastingMode.On : ShadowCastingMode.Off;
    ...
    void GeneratePrimitive()
    {
        ...
        primitive.GetComponent<MeshRenderer>().shadowCastingMode = shadows ? ShadowCastingMode.On : ShadowCastingMode.Off;
    }
    void GeneratePositions()
    {
        ...
        for (int y = -1; y <= 1; y++)
        for (int z = -1; z <= 1; z++)
            if ((type == "Sponge" && x * y + y * z + z * x != 0) || (type == "Cloud" && x == 0 ^ y == 0 ^ z == 0 && (x != 0 || y != 0 || z != 0)) || (type == "Snowflake" && x + y == 0 ^ y + z == 0 ^ z + x == 0))
    ...
    void GenerateStep(Transform parent, int step = 1)
    {
        foreach (var position in positions)
        {
            ...
            if (step != steps) GenerateStep(child, step + 1);
        }
    }
    ...
    void OnDrawGizmos()
    {
        if (playAnimation) PlayAnimation(transform);
