...
public class VInspectorData : ScriptableObject
{
    ...
    public void Setup(Object target)
    {
        void buttons()
        {
            ...
            foreach (var member in membersWithButtonAttributes) createButton(member, member.GetCustomAttribute<ButtonAttribute>());
        }
        ...
        void tabs()
        {
            ...
            void setupTab(Tab tab, IEnumerable<string> allSubtabPaths)
            {
                void repair()
                {
                    if (tab.subtabs == null) tab.subtabs = new List<Tab>();
                ...
                void setupSubtabs()
                {
                    foreach (var subtab in tab.subtabs) setupTab(subtab, allSubtabPaths.Where(r => r.StartsWith(subtab.name + "/")).Select(r => r.Remove(subtab.name + "/")).ToList());
                }
        ...
        void foldouts()
        {
            ...
            void setupFoldout(Foldout foldout, IEnumerable<string> allSubfoldoutPaths)
            {
                void repair()
                {
                    if (foldout.subfoldouts == null) foldout.subfoldouts = new List<Foldout>();
                ...
                {
                    foreach (var subtab in foldout.subfoldouts) setupFoldout(subtab, allSubfoldoutPaths.Where(r => r.StartsWith(subtab.name + "/")).Select(r => r.Remove(subtab.name + "/")).ToList());
                }
    ...
    public class Tab
    {
        ...
        public void ResetSubtabsDrawn()
        {
            ...
            foreach (var r in subtabs) r.ResetSubtabsDrawn();
        }
    ...
    public class Foldout
    {
        ...
        public bool IsSubfoldoutContentVisible(string path)
        {
            ...
            else if (!path.Contains('/')) return expanded && subfoldouts.Find(r => r.name == path).expanded;
            else return expanded                          && subfoldouts.Find(r => r.name == path.Split('/').First()).IsSubfoldoutContentVisible(path.Substring(path.IndexOf('/') + 1));
        }
